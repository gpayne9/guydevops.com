...
<img src="/assets/img/cf-static-site-post/terraform.svg" alt="Terraform Logo" width="100" height="300" />
...
7. To build the statically generated files locally we will just run `jekyll build`. The output is in the `_site` folder. This will be need to be ran before any deployment to pick up the latest changes to your files.

8. Now that we have the project set up you can edit the values in the `_config.yml` and the `.mb` files in the `_post` directory to create articles.
...
2\. Next we need to initialize the terraform backend before we start writing more terraform. Run `terraform init`.

2\. Now lets create more terraform files. Create a file named `s3.tf` for the S3 bucket with s3 website configuration.
  ...
  - The site deployment to s3 is done via a ```null_resource``` block. This automates the process of copying/synching the files generated by `jekyll build`.
...
3\. Now lets create the ACM SSL certificate and use DNS validation in a file called `acm_cert.tf`. The ```aws_route53_zone``` can be imported via a terraform data lookup.
...
4\. Finally lets create the CloudFront Distrubution in a file called `cloudfront.tf`
...
resource "aws_cloudfront_distribution" "guydevops_cf_dis" {
  ...
  origin {
    ...
    # Add a custom secret_header that will be sent to S3 to allow the CloudFront distribution access to read from the S3 Bucket
    custom_header {
      name  = "Referer"
  ...
  # This configuration redirects all request to the S3 Origin
...
5\. Next lets create the static files with the `jekyll build` if you haven't done so already. This should output the files in the `_site` folder which should be the same one we configured in the `site_path` terraform variable.

6\. Now we run `terraform plan` to see all of the changes and additions to the infrastructure.

7\. Finally run `terraform apply` and all the infrastructure should take a few minutes to create.

## Conclusion

Now you have a easy to edit blog to write anything you want in Markdown with the resiliency and reusability of terraform 
